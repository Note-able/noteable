...
module.exports = function (app, options) {
  ...
  var uploadPicture = function uploadPicture(req, res, next) {
    ...
    form.parse(req, function (err, fields) {
      ...
      image.sendUploadToGCS(splits[splits.length - 1], buffer).then(function (response) {}).catch(function (error) {
        console.log(error);
      });
    });
  };
  ...
  app.post('/post-blob', function (req, res) {
    ...
    form.parse(req, function (err, fields) {
      ...
      audio.sendUploadToGCS('.mp3', buffer).then(function (result) {
        m_mediaSerivce.createMusic({ audioUrl: result.cloudStoragePublicUrl, author: req.user.id, createdDate: new Date().toISOString(), name: fields.name, size: fields.size }).then(function () {
          res.status(204).send();
        }).catch(function (error) {
          res.json(error);
        });
