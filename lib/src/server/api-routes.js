...
module.exports = function (app, options) {
  ...
  app.post('/conversations', options.auth, function (req, res) {
    ...
    m_messageService.createConversation(userIds, function (conversationId, error) {
      if (error != null) {
        res.status(500).send();
      }
  ...
  app.get('/conversations', options.auth, function (req, res) {
    if (!req.user) {
      ...
      m_messageService.getConversationsByUserId(req.user.id, function (conversations, error) {
        if (error != null) {
          res.json(error);
          res.status(500).send();
        } else {
  ...
  app.get('/conversation', options.auth, function (req, res) {
    if (!req.user) {
      res.status(404).send();
    } else if (req.query.conversationId == null) {
      res.status(400).send();
    } else {
      m_messageService.getConversation(req.query.conversationId, req.user.id, function (conversation, error) {
        if (error != null) {
  ...
  app.get('/message', options.auth, function (req, res) {
    if (!req.user) {
      res.status(404).send();
      return;
    }
    ...
    m_messageService.getMessage(req.query.messageId, req.user.id, function (message, error) {
      if (error != null) {
        res.json(error);
        res.status(500).send();
        return;
  ...
  app.get('/messages', options.auth, function (req, res) {
    ...
    if (req.user == null) {
      ...
      m_messageService.getMessages(req.user.id, req.query.conversationId, req.query.start, req.query.count, function (messages, error) {
        if (error != null) {
          res.json(error);
          res.status(500).send();
        } else {
  ...
  app.post('/messages', options.auth, function (req, res) {
    ...
    m_messageService.createMessage(req.body.conversationId, req.body.userId, req.body.content, req.body.destinationId, function (messageId, error) {
      if (error != null) {
        res.json(error);
        res.status(500).send();
        return;
  ...
  app.delete('/message', options.auth, function (req, res) {
    if (req.user == null) {
      res.status(404).send();
      return;
    ...
    m_messageService.deleteMessage(req.query.messageId, function (count, error) {
      if (error != null) {
        res.json(error);
        res.status(500).send();
        return;
  ...
  app.delete('/conversation', options.auth, function (req, res) {
    if (req.user == null) {
      res.status(404).send();
      return;
    } else if (req.query.conversationId == null) {
    ...
    m_messageService.deleteConversation(req.query.conversationId, function (count, error) {
      if (error != null) {
        res.json(error);
        res.status(500).send();
        return;
