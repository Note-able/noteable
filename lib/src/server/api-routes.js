...
module.exports = function (app, options) {
  ...
  var m_mediaSerivce = new _services.MediaService(options);
  var m_notificationService = new _services.NotificationService(options);
  var m_newsfeedService = new _services.NewsfeedService(options);
  ...
  app.get('/music/user', function (req, res) {
    if (!req.user) {
    ...
    m_mediaSerivce.getMusicByUser(req.user.id, options).then(function (result) {
  ...
  app.post('/post-blob', options.auth, function (req, res) {
    var form = new Formidable.IncomingForm();
    ...
    form.parse(req, function (err, fields, files) {
      ...
      audio.sendUploadToGCS(fields.extension ? fields.extension : '.mp3', buffer).then(function (result) {
        m_mediaSerivce.createMusic({ audioUrl: result.cloudStoragePublicUrl, author: req.user.id, createdDate: new Date().toISOString(), name: fields.name, size: fields.size }).then(function (id) {
  ...
  app.get('/newsfeed/:destinationId', options.auth, function (req, res) {
    ...
    m_newsfeedService.getNewsfeed(req.params.destinationId, req.query.limit, req.query.offsetId).then(function (items) {
  ...
  app.get('/notifications', options.auth, function (req, res) {
    ...
    if (req.query != null && req.query.ids == null) {
      m_notificationService.getNotificationsByUser(req.user.id, req.query.limit || DEFAULT_QUERY_SIZE, req.query.offsetId || 0, req.query.status || 0).then(function (notifications) {
        return res.json(notifications);
      }).catch(function (error) {
        return res.json(error);
      });
    } else {
      m_notificationService.getNotifications(req.query.ids, req.query.limit || DEFAULT_QUERY_SIZE, req.query.status || 0).then(function (notifications) {
        return res.json(notifications);
      }).catch(function (error) {
  ...
  app.get('/notifications/:notificationId', options.auth, function (req, res) {
    ...
    m_notificationService.getNotification(req.params.notificationId).then(function (notification) {
  ...
  app.post('/notifications/markread', options.auth, function (req, res) {
    ...
    m_notificationService.markNotificationsAsRead(req.user.id, req.query.ids).then(function () {
  ...
  app.post('/notifications/delete/:notificationId', options.auth, function (req, res) {
    ...
    m_notificationService.deleteNotification(req.params.notificationId).then(function () {
      return res.status(204).send();
    }).catch(function (error) {
      return res.json(error);
    });
  ...
  app.post('/conversations', options.auth, function (req, res) {
    ...
    m_messageService.createConversation(userIds).then(function (result) {
