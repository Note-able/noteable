...
var MessageService = function () {
  ...
  _createClass(MessageService, [{
    ...
    value: function createConversation(userIds) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this.options.connect(_this.options.database, function (connection) {
          ...
          _this.getConversationByUserIds(userIds[0], userIds[1]).then(function (result) {
            if (result) {
              resolve(result);
              return;
            }
    ...
    key: 'getConversationByUserIds',
    value: function getConversationByUserIds(userId, otherUserId) {
      var _this4 = this;
        ...
        _this4.options.connect(_this4.options.database, function (connection) {
          ...
          connection.client.query('\n          SELECT c1.user_id, c1.last_read_message, c1.conversation_id, c1.id, c1.is_deleted, c2.user_id as other_user_id\n          FROM conversations c1\n          INNER JOIN conversations c2\n          ON c2.conversation_id = c1.conversation_id\n          WHERE c1.user_id = ' + userId + ' and c2.user_id = ' + otherUserId + ';\n        ').on('row', function (row) {
            ...
            if (conversation.length !== 0) {
              return _this4.getConversation(conversation[0].conversation_id, userId).then(function (result) {
                resolve(result);
              });
            }
    ...
    value: function deleteConversation(conversationId) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this9.options.connect(_this9.options.database, function (connection) {
          if (connection.client == null) {
            reject('Failed to connect to database.');
            return;
          }
