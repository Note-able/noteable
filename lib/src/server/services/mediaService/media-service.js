...
var MusicService = function () {
  ...
  _createClass(MusicService, [{
    ...
    key: 'getMusicByUser',
    value: function getMusicByUser(userId, options) {
      var _this2 = this;

      return new Promise(function (resolve, reject) {
        _this2.options.connect(_this2.options.database, function (connection) {
          var result = [];
          var query = 'SELECT ' + (0, _musicDto.columns)('m', 'SELECT') + ' WHERE m.author = ' + userId + (' LIMIT ' + (options.limit || defaultMusicLimit)) + (' OFFSET ' + (options.offset || 0)) + ';';
          console.log(query);
