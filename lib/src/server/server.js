...
if (_fs2.default.existsSync('./src/server/keys/server.key')) {
  (function () {
    var options = {
      key: _fs2.default.readFileSync('./src/server/keys/server.key'),
      cert: _fs2.default.readFileSync('./src/server/keys/server.crt'),
      requestCert: false,
      rejectUnauthorized: false
    };

    var httpsServer = _https2.default.createServer(options, app).listen(443, function () {
    ...
    require('./sockets')(httpsServer, { auth: _serverUtil.ensureAuthenticated, connect: _serverUtil.connectToDb, database: config.connectionString });
  })();
}
...
require('./sockets')(httpServer, { auth: _serverUtil.ensureAuthenticated, connect: _serverUtil.connectToDb, database: config.connectionString });