...
module.exports = function (app, options) {
  ...
  app.post('/register', (req, res) => {
    ...
    bcrypt.hash(req.body.password, null, null, (err, password) => {
      ...
      options.connect(options.database, connection => {
        connection.client.query(`INSERT INTO public.user (username, password) VALUES ('${ req.body.email }', '${ password }');`).on('error', error => {
          res.send(error);
        }).on('end', () => {
          res.status(204).send();
          connection.fin();
    ...
    options.connect(options.database, connection => {
      ...
      connection.client.query(`
        SELECT * FROM public.profile pr
          JOIN public.pictures pic
          ON pic.user_id = pr.id
        WHERE ${ req.params.id } = pr.id AND pic.picture_type = 1;`).on(`row`, row => {
        user.push(row);
      }).on(`error`, error => {
