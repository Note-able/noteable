  ...
  app.post('/document/:documentId', options.auth, (req, res) => {
    ...
    options.connect(options.database, connection => {
      ...
      connection.client.query(`SELECT id, title, description, date, modified, profiles FROM public.documents
        ...
        console.log(`pushing row ${ row }`);song.push(row);
      }).on(`error`, error => {
        ...
        if (song.length > 0) {
          ...
          connection.client.query(`INSERT INTO documents (contents, date, modified, profiles) VALUES ('${ req.body }', current_timestamp, current_timestamp, '{${ req.user.id }}');`).on(`row`, row => {
            song.push(row);
          }).on(`end`, () => {
            res.status(200).send();
            connection.fin();
