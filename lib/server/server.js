...
passport.use(new LocalStrategy({ callbackURL: '/auth/local/callback'
}, (username, password, done) => {
  ConnectToDb(config.connectionString, connection => {
    if (connection.status === `SUCCESS`) {
      let user = null;
      connection.client.query(`SELECT * FROM public.user WHERE email = '${ username }';`).on(`row`, row => {
        user = row;
        connection.fin();
        if (!user) {
...
passport.use(new FacebookStrategy({
  ...
  ConnectToDb(config.connectionString, connection => {
    if (connection.status === `SUCCESS`) {
      ...
      connection.client.query(`SELECT * FROM public.user WHERE facebook_id = '${ profile.id }';`).on(`row`, row => {
        user = row;
        connection.fin();
        return done(null, user);
      }).on('end', () => {
...
app.get('/auth/local', passport.authenticate('local', { failureRedirect: '/login' }), (req, res) => {
  res.redirect('/');
});
