  ...
  render() {
    ...
    if (this.props.chords) {
      ...
      const chords = sortedChords.map(chord => {
        ...
        content.push(React.createElement(TextBlock, { text: text, index: lastTextIndex }));
        lastTextIndex = chord.index;
        return React.createElement(Chord, { key: `chord${ chord.index }`, ref: `chord${ chord.index }`, index: chord.index, text: chord.text || '', updateSelectedToTextLine: () => this.props.updateSelected(this.props.lineId) });
      });
      content.push(React.createElement(TextBlock, { text: this.props.text.substring(lastTextIndex, this.props.text.length), index: lastTextIndex }));
