  ...
  render() {
    ...
    if (this.props.chords) {
      ...
      const chords = sortedChords.map(chord => {
        ...
        content.push(text);
        return React.createElement(Chord, { key: `chord${ chord.index }`, ref: `chord${ chord.index }`, index: chord.index, text: chord.text || '', updateSelectedToTextLine: () => {
            chord.updateSelectedFunction();
          } });
      });
