...
let Section = function (_React$Component) {
  ...
  _createClass(Section, [{
    ...
    value: function showTooltip(selection) {
      ...
      const tooltip = ReactDOM.render(React.createElement(Tooltip, { element: selection.anchorNode.parentElement, rect: rect, addChord: this.addChord.bind(this), range: range }), this.refs.tooltip);
    ...
    value: function addChord() {
      ...
      const chord = ReactDOM.render(React.createElement(Chord, { character: character, updateSelectedToTextLine: () => {
          this.props.dispatch(_updateSelected(this.props.sectionId, this.props.section.selectedIndex, range.endOffset));
        } }), container);
