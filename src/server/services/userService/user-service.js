...
export default class UserService {
  ...
  updateUser = async (userId, facebookId) =>
    new Promise(async (resolve, reject) => {
    ...
    new Promise(async (resolve, reject) => {
      try {
        ...
        if (profile.instruments != null) {
          const instrumentSets = profile.instruments.reduce(
            (obj, instrumentId) => {
        ...
        if (profile.preferences != null) {
          await connection.execute(
            ...
            WHERE profile_id = :id;`,
            {
              isLooking: profile.preferences.isLooking ? 1 : 0,
              displayLocation: profile.preferences.displayLocation ? 1 : 0,
              id
            }
          );
