module.exports = function (app, pg, options) {
  ...
  app.get(`/user/{id}/profile`, options.auth, function (req, res) {
  ...
  app.get(`/events/filter/{type}/{instrument}/{index}/{filter}`, options.auth, function (req, res) {
    if (req.params.instrument.length === 0 && req.params.type.length === 0) {
      ...
      options.connect(options.database, function (connection) {
        ...
        connection.client
        .query(`SELECT * FROM public.events WHERE ` + query + ` LIMIT 10 OFFSET ` + 10 * index + ` ORDER BY ` + req.params.filter + ` DESC;`)
        .on(`row`, function (row) {
          events.push(row);
        }).on(`end`, function (result) {
          res.send(events);
